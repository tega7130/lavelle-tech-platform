// ISO 3166-1 alpha-2 -> flag emoji, computed rather than hand-typed —
// each Unicode regional indicator symbol is 127397 codepoints above its
// ASCII letter, so two of them together render as that country's flag
// in any font that supports the sequence. Avoids 190+ hand-copied emoji
// (typo/mojibake risk) for what's otherwise just two lookups.
...
// Dial (calling) codes per ITU-T E.164. Several countries share one —
// e.g. every NANP member (US, Canada, most Caribbean nations) is +1 —
// which is expected: the field stores the dial code itself, not which
// specific country was picked, same as before this list was expanded.
