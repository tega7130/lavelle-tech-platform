...
import { savePosition } from "@/lib/player-actions";
 ...
 * sendBeacon can only POST to a plain URL. The actual write lives in
 * src/lib/player-actions.ts's savePosition — extracted so it's testable
...
export async function POST(request: NextRequest) {
  ...
  const { enrolmentId, lectureId, slideIndex, mediaPositionSeconds } = parsed.data;
