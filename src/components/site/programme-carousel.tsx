...
function CarouselCard({ p, index }: { p: CarouselListing; index: number }) {
  const { ref, inView } = useInView<HTMLAnchorElement>({ rootMargin: "0px" });
  return (
    <Link
      ref={ref}
      href={`/programmes/${p.code}`}
      className={cn(
        "lv-reveal group block flex-none w-[300px] snap-start bg-bg border border-divider rounded-[14px] p-[26px] no-underline text-text transition hover:-translate-y-1 hover:shadow-[0_18px_40px_rgba(19,26,46,0.1)] hover:border-accent-200",
        inView && "lv-in"
      )}
...
export function ProgrammeCarousel({ listings }: { listings: CarouselListing[] }) {
  ...
  return (
    <>
      ...
      >
        {listings.map((p, i) => (
          <CarouselCard key={p.code} p={p} index={i} />
        ))}
      </div>
